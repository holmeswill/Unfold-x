...
BC = Triclinic('Na',latticeconstant=(3,3.1,3.2,77,99,121))

TRMAT=np.array([[1,1,0],
                [-1,1,0],
                [0,0,2]])
...
band_input_data['SYSTEM']['nbnd'] = 44
...
&inputun
  ...
  outdir='./out/'                    ! output files directory
  Emin=0                             ! min energy for spectral function    
  ...
  write_pkm=.true.                   ! output P_Km coefficients and corresponding eigenvalues. WARNING: nspin x nkpt files will be created!
...
cd BC
$QE_ROOT/bin/pw.x -inp {PREFIX}.scf > {PREFIX}.scf.out
$QE_ROOT//bin/pw.x -inp {PREFIX}.bands > {PREFIX}.bands.out
$QE_ROOT//bin/bands.x -inp {PREFIX}.pp_band > {PREFIX}.pp_band.out
cd ../SC
$QE_ROOT//bin/pw.x -inp {PREFIX}.scf > {PREFIX}.scf.out
../../../bin/unklist.x < {PREFIX}.unklist > {PREFIX}.unklist.out
...
$QE_ROOT//bin/pw.x -inp {PREFIX}.bands_crystal > {PREFIX}.bands_crystal.out
../../../bin/unfold.x < {PREFIX}.unfold > {PREFIX}.unfold.out 
cd .. && python test.py && cd -
$QE_ROOT//bin/pw.x -inp {PREFIX}.bands_tpiba > {PREFIX}.bands_tpiba.out
../../../bin/unfold.x < {PREFIX}.unfold > {PREFIX}.unfold.out 
cd .. && python test.py && python plot.py test
rm -r BC/out SC/out
